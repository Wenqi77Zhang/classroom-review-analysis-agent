...
| 认证与课程/课堂 API | **部分实现**：登录、可选演示账号、`/auth/me`、课程创建/列表、课堂创建/列表/读取/修改 |
...
- `docs/interface-contracts.md` 中认证、课程创建/列表及课堂创建/列表/读取/修改端点已实现；其余业务端点仍只是契约或明确禁用。
- 上传、任务、逐字稿、分析和报告路由及其领域服务尚未实现；认证及已列出的课程/课堂基础 API 可调用。
- 持久化、登录 HTTP 路由和 owner-scoped 课堂流程已有真实 PostgreSQL 测试；任务状态机服务仍未覆盖。
