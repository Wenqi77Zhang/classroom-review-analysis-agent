            ...
            <section className="conversation-panel" data-reveal aria-label="与 Agent 对话">
              ...
              <div className="messages" aria-live="polite">
                ...
                {contract && (
                  <article className="message agent">
                    <div className="message-label">Agent</div>
                    <p>我已根据两轮输入整理为右侧分析契约。请核对范围、证据条件和隐私边界后再确认。</p>
                  </article>
                )}
              </div>
              ...
              <form className="composer" onSubmit={send}>
                ...
                <textarea
                  ...
                  placeholder={conversationStep === 0 ? "例如：请分析内容组织、讲解清晰度和提问后的等待时间……" : conversationStep === 1 ? "例如：分析整节课堂，每条判断保留时间戳和中英原文……" : "分析契约已形成；如需修改，请重新开始本地演示。"}
                />
                <div className="composer-footer">
                  <button
                    ...
                    type="button"
                    disabled={conversationStep === 2}
                    onClick={() => setGoal(conversationStep === 0 ? "请分析整节课堂的内容组织、讲解清晰度和提问后的等待时间，并为每条判断附上原文证据。" : "分析整节课堂；每条判断必须保留时间戳、课堂原文和中文翻译。")}
                  >
                    使用示例回答
                  </button>
                  <button className="button primary compact" type="submit" disabled={conversationStep === 2}>
                    {conversationStep === 0 ? "发送目标" : "回答追问"}
                  </button>
                </div>
              </form>
            </section>
