...
assert.match(player, /尚未取得课堂视频的授权地址/, "不得伪造对象存储接入");
...
assert.match(task, /requireSession|redirectToLogin/, "任务页必须通过服务端会话接口恢复访问");
assert.match(task, /登录并返回/, "任务页必须提供可操作的会话恢复入口");
assert.match(realWorkbench, /error\?\.status === 401/, "证据工作台必须识别会话过期");
assert.match(realWorkbench, /requireSession|redirectToLogin/, "证据工作台必须能够重建安全会话");
assert.match(realWorkbench, /登录并返回/, "证据工作台必须提供会话恢复按钮");
