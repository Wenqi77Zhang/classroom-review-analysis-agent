...
2026-09-11：核心工作流已接通真实服务，并完成本轮报告、故障恢复、登录与两轮对比改造。
当前版本仍待正式部署与独立教师验收，不能将工程测试等同于教学效果。

| 能力 | 当前实现与验证边界 |
|---|---|
| M1 单节课堂复盘 | 本地模型确认分析契约，真实视频上传、处理、证据定位、教师复核、报告编辑和三格式导出。既有两段不同真实课程视频技术 E2E 保留在验收记录中；本轮未重新运行视频 ASR |
| 报告编辑 | 标题与已确认结论可保存，正文修改追加教师复核历史；原文、时间/页码和模型来源随报告导出。并发修改冲突返回 409，保留当前草稿 |
| 故障恢复 | 重试和取消写入后端任务；刷新读取实际课堂与任务；过期会话要求显式登录并返回原资源 |
...
公开发布仍需获授权媒体、非开发教师独立试用、受控主机与稳定 HTTPS 入口，并完成干净环境复现。
