...
| M3 多课程总览 | AWS 已显示 2 门课程、3 节课堂及 1 个符合门禁的真实循环；汇总显式标记该循环为“证据不足”，不把纳入记录写成教学改善。完整 M3 效果验收仍需至少两门课程各自完成独立真实教学闭环 |

最新后端回归为 `380 passed, 12 skipped`；12 项需要显式外部集成环境或真实媒体，未被伪装成通过。
