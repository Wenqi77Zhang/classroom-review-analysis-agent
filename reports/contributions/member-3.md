...
- 当前分支待合并：教师接受/修改/驳回和版本历史 API、报告保存/读取与对应最小审计。
- PR #20 已由成员 1 协作完成 Worker/B2 下载、大小/ETag 校验和时间戳逐字稿写回
  PostgreSQL 的真实单输入验收；该项不改写为成员 3 独立成果。
...
- 实现课堂报告读取/保存；后端只关联当前为 `accepted` 或 `modified` 的结论，驳回会
  移除既有报告关联。
