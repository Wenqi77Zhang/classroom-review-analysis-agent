...
实现边界：`analyses` 已实现结论读取、教师复核 append-only 历史与 Agent 内部批量写入；
`reports` 已实现 `GET|PUT /classrooms/{id}/report` 持久化草稿，关联结论由后端从
`accepted/modified` 状态计算，客户端不能提交结论 ID 绕过门禁。报告导出端点仍为
`TODO`，端点列入冻结契约不等于已经可以调用。
