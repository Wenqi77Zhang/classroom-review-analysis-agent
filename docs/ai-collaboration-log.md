...
| 2026-07-30 | 成员 1 + Codex | PR #27 Worker 合并前安全复核与修正 | 审核成员 4 媒体流水线是否满足证据来源、进程停机、课件隐私和资源限制门禁；保留原成果归属 | AI 审查差异、构造边界回归并协助最小修复；未代替成员 4 的流水线设计与主体实现 | 成员 1 要求审核后直接修正易改阻断项；自动化执行 Worker/翻译/证据/领域规则 112 项测试，并用独立 Python 运行验证 PPTX 讲者备注排除和异常压缩拒绝 | 原实现把同一逐字稿片段同时登记为 `TRANSCRIPT` 与 `VIDEO`，可绕过视觉证据门禁；进程停止事件未传入活动租约；PPTX 讲者备注会进入证据且缺少解压规模门禁；贡献报告部分描述超出实现事实 | 逐字稿只生成逐字稿证据；活动任务在安全检查点响应进程停止；PPTX 仅抽取幻灯片可见内容并拒绝超大解压体积/异常压缩比；同步纠正成员 4 贡献报告，完整课件 pytest 因临时依赖下载未完成，不能虚报为已执行 | PR #27、`worker/runner.py`、`worker/stages/build_evidence_index.py`、`worker/stages/parse_courseware.py`、对应单元测试 |
