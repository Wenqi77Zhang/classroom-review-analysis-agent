...
|---|---|---|---|---|---|---|---|---|
| 2026-07-30 | 成员 3 + Codex | MVP Trace、幂等、恢复与权限加固 | “继续实现 Trace 和审计 API；补重启恢复、幂等性和权限验证”；“修复旧契约毒任务与并发 event_id” | 增加白名单 Agent Trace、教师审计读取、事件幂等键和恢复回归；复审后增加旧契约任务隔离及跨任务唯一键竞争的保存点处理 | 审核基线真实 PostgreSQL 为 `183 passed, 1 skipped, 1 failed`，GitHub CI 为 `180 passed, 5 skipped`；失败明确复现旧契约毒任务。修复和两项 PostgreSQL 回归已提交但未在正在运行的真实链路数据库上重复执行，等待新 CI/负责人复验 | 初版在响应序列化阶段才发现旧 JSON 不兼容，事务回滚导致任务无限重领；不同任务锁无法串行化全局 `event_id` | 不可转换任务落稳定、脱敏失败并释放租约；`event_id` 插入使用保存点将并发唯一键竞争映射为 `STATE_CONFLICT`；不删除真实旧任务证据 | `backend/app/api/tasks.py`、`backend/app/repositories/audit.py`、`tests/integration/test_processing_api.py` |
