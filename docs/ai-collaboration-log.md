...
| 2026-07-29 | 成员 3 | 登录与课程/课堂 CRUD | “从 PR #10 派生后续分支；按成员 1 已确认页面字段实现真实登录与课堂 API” | 认证/身份 Schema、身份仓储、6 组 API 路径及真实 HTTP 隔离测试 | OpenAPI 路径检查；临时 PostgreSQL 执行迁移后跑完整登录→课程→课堂→越权→修改→删除流程 | 首版 `api/auth.py` 与 dependencies 形成循环依赖，拆出 authentication 服务；PATCH 后数据库生成的 updated_at 触发 MissingGreenlet，增加显式 refresh | 修改后采纳；授课语言/日期因产品语义未冻结而未擅自入库 | `backend/app/api/auth.py`、`backend/app/api/classrooms.py`、`tests/integration/test_auth_classroom_api.py` |
