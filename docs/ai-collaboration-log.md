...
|---|---|---|---|---|---|---|---|---|
| 2026-07-30 | 成员 3 + Codex | MVP 写操作审计完整性 | “完成 MVP 后续所有工作” | 补齐课程/课堂、上传核验、教师任务操作和逐字稿写入/编辑的白名单审计，并把任务相关事件绑定任务 Trace 供既有审计 API 查询 | PostgreSQL 定向用例覆盖课堂写入、失败上传事务、任务审计、跨账号和重启恢复；全量门禁见对应 PR | 首版事件虽入库但使用当前 HTTP Trace，无法由任务审计 API 找回；测试覆盖又错误地在所有异常上回滚，未镜像生产 `commit_changes` 语义 | 审计助手支持显式任务 Trace；任务事件统一使用 `processing_task`；测试事务镜像生产依赖，保留审计外键限制 | `backend/app/services/audit.py`、`backend/app/api/`、`tests/integration/test_processing_api.py` |
| 2026-07-30 | 成员 3 + Codex | MVP 服务端报告导出 | “继续下面工作并提交 PR” | 在 PR #24 上叠加 Markdown/HTML/PDF 渲染、S3 Provider 服务端写入、版本化对象 key、限时下载与导出审计 | PostgreSQL HTTP 与渲染测试通过；后端全量 186 项通过/1 项条件跳过，Ruff、迁移漂移、前端测试/类型/构建通过；PDF 渲染 PNG 人工检查中文可见且无裁切 | 初版从数据库引擎读取签名 TTL 且 GET 为确定扩展名重复渲染 PDF；自查发现后改为应用配置和静态格式元数据。进一步补充正文/标题变化时清空数据库导出指针 | 导出前锁定课堂并按当前复核状态重建正文；旧版本不再签发新 URL，存储生命周期负责物理清理；前端交互仍归成员 1 | `backend/app/api/reports.py`、`backend/app/services/report_exports.py`、`tests/integration/test_review_report_api.py` |
| 2026-07-30 | 成员 3 + Codex | MVP Trace、幂等、恢复与权限加固 | “继续实现 Trace 和审计 API；补重启恢复、幂等性和权限验证”；“修复旧契约毒任务与并发 event_id” | 增加白名单 Agent Trace、教师审计读取、事件幂等键和恢复回归；复审后增加旧契约任务隔离及跨任务唯一键竞争的保存点处理 | 审核基线真实 PostgreSQL 为 `183 passed, 1 skipped, 1 failed`，失败明确复现旧契约毒任务；修复后由成员 1 在隔离的 PostgreSQL 17 临时实例复验为 `184 passed, 1 skipped`；再合入 `main@c3c92d6` 的 PR #27 状态机回归并解决冲突后，全仓 Ruff 与组合 PostgreSQL 门禁为 `265 passed, 1 skipped` | 初版在响应序列化阶段才发现旧 JSON 不兼容，事务回滚导致任务无限重领；不同任务锁无法串行化全局 `event_id` | 不可转换任务落稳定、脱敏失败并释放租约；`event_id` 插入使用保存点将并发唯一键竞争映射为 `STATE_CONFLICT`；成员 1 仅协助真实数据库复验及 PR #27 合入后的冲突整合，不改写成员 3 功能归属 | `backend/app/api/tasks.py`、`backend/app/repositories/audit.py`、`tests/integration/test_processing_api.py` |
| 2026-07-30 | 成员 5 + Codex | 同步并核验已合并 PR #27 | “PR #27 在 25 分钟前有修复更新” | 核对 PR 最终提交与合并状态，将 `main@c3c92d6` 普通合并进 PR #31，保留双方协作日志并复核专业 Skill、证据门禁和 Worker 阶段 | PR #27 状态为 merged/approved/CI 全绿；合并后专项 `63 passed`，完整门禁 Python `265 passed, 11 skipped`，Ruff、前端测试、类型检查和构建通过 | PR #31 原进度记录仍把成员 4 专业能力列为 `TODO`，且分支未包含 PR #27 最终兼容修复 | 专业 Skill 与证据校验器改记为自动化通过；真实翻译 Provider、后端证据写回、第二远程输入和浏览器 E2E 继续如实保留未完成 | PR #27、PR #31、`docs/current-progress.md`、`docs/acceptance-matrix.md`、`tests/manual/member-5-release-gate-record.md` |
| 2026-07-30 | 成员 5 + Codex | PR #31 已合并契约与团队入口保护 | “按以上建议推进” | 根据复审和 PostgreSQL 失败栈识别源码快照覆盖 PR #22/#30 已合并成果；恢复报告服务端正文门禁、并发锁、审计脱敏、专用测试和 team-access/隧道文件，同时保留统一启动说明 | GitHub PostgreSQL CI `190 passed, 5 skipped`，scaffold、Ruff、前端 CI 全绿；恢复团队入口后本机完整门禁 `184 passed, 11 skipped` 且 `TEAM_TUNNEL_ACCESS_GATE_OK` | 未经负责人确认删除已合并跨模块成果违反范围与责任边界；根目录参考说明不能替代成员 1 确认 | 以 `origin/main` 为基线恢复成员 3 后端契约和成员 1 团队入口，再叠加成员 5 Agent/Trace/统一启动改动 | PR #31、`backend/app/repositories/reviews.py`、`frontend/tests/team-access.test.mjs`、`docs/local-development-setup.md` |
| 2026-07-30 | 成员 5 + Codex | PR #31 CI 阻断修复 | “按以上建议推进” | 定位 scaffold 在未创建 `.venv` 时误跑完整门禁，以及 PostgreSQL 测试清理未先删除审计事件；恢复被删的复核/报告专用测试 | 定向 `4 passed, 2 skipped`、Ruff 通过；完整本机门禁 `184 passed, 11 skipped`，前端测试、类型检查和生产构建通过 | 完整 `verify.sh` 已从阶段 0 骨架检查扩展为发布门禁，但 CI scaffold 仍沿用旧调用方式；新增审计外键后最短链路测试清理顺序未同步 | scaffold 使用显式 `--scaffold-only`；测试按外键顺序清理审计事件；保留专用报告门禁回归，等待 GitHub PostgreSQL CI 实跑 | `.github/workflows/ci.yml`、`verify.sh`、`tests/integration/test_processing_api.py`、`tests/integration/test_review_report_api.py` |
| 2026-07-30 | 成员 5 + Codex | 发布门禁与剩余链路复核 | “继续完成刚刚的任务，后续未完成任务也要完成” | 升级跨平台 verify/README 门禁，复核复核历史/报告后端代码、前端接线、成员 4 Skill、基础设施和浏览器可用性，并同步事实源 | `verify.ps1`：Python `183 passed, 10 skipped`，Ruff、前端测试、TypeScript、Next.js 构建通过；README 门禁复跑通过；浏览器发现为空 | 当前源码快照无 `.git`，无 PostgreSQL/Docker；后端 API 已出现但前端仍走 Mock 和 `/reports/demo`；不能把测试代码、合成证据或隧道地址冒充真实网页全链路 | 采纳本机可执行门禁；复核/报告与统一启动保留“局部通过”，真实视频、数据库、浏览器 E2E 和成员 4 专业能力继续阻塞；访问码不入库，隧道建设按 `跳过.md` 跳过 | `verify.ps1`、`verify.sh`、`scripts/verify-readme.*`、`tests/manual/member-5-release-gate-record.md` |
| 2026-07-30 | 成员 5 + Codex | Trace 持久化与统一启动入口 | “后续未完成任务也要完成；只完成成员 5，沿用其他成员接口” | 审计现有 Trace/前端链路，补原文类字段全量脱敏；将阶段 0 启动占位替换为前端、后端、Worker、Agent 四服务入口及安全轮询包装 | Trace/后端 `113 passed`、前端完整测试通过；PowerShell 语法、启动静态测试通过、缺 `.env` fail-closed 实测通过 | 发现 JSONL 会保留截断后的逐字稿；Worker/Agent 一次领取入口不能直接作为常驻服务；服务令牌不得进入 CLI | 修复后采纳；Trace 自动化验收通过；统一启动代码完成但真实基础设施复验继续标局部通过；成员 3 报告 API 与成员 4 Skill 占位未越权修改 | `agent/observability/tracing.py`、`start.ps1`、`start.sh`、`scripts/run_service_loop.py`、`tests/unit/test_start_scripts.py` |
| 2026-07-30 | 成员 5 + Codex | 接入本地 Ollama `qwen3.5:4b` | “运行时 Ollama；模型 qwen3.5:4b；如果没有可以安装” | 安装模型，复用现有本地 Provider 与环境变量；增加可选 reasoning 控制和 Ollama 生成期 Schema 兼容层，保留完整 Pydantic/证据落地校验 | 真实 HTTP 简单 Schema 成功；完整 Agent 首轮复现空正文和 grammar 400，修复后真实模型生成 3 条结构化结论，模型/Prompt/Skill/Trace 与毫秒证据范围均通过；定向测试 `26 passed`，Ruff 通过 | Qwen thinking 默认开启；Ollama 0.32.5 无法把 Pydantic 的格式、描述和长度元数据编译为 grammar。不能删除落地校验，也不能把合成证据冒充真实视频逐字稿 | 仅弱化本地模型的生成指导 Schema，完整输出校验不变；真实模型接入采纳，真实视频逐字稿、数据库写回与完整 P0 继续 `TODO`；临时访问码不入库 | `agent/providers/base.py`、`agent/providers/local.py`、`agent/runner.py`、`tests/unit/test_agent.py`、`reports/contributions/member-5.md` |
| 2026-07-30 | 成员 4 + Codex | PR #27 兼容优先最小修复 | “无真实 TranslationAdapter 时不进入 translate；修复 transcribe 租约重领倒退；中文课件不被双语门禁误拒；增加真实 FastAPI 状态机组合测试” | 将远程主链恢复为 transcript-first；增加 claimed stage floor 并覆盖成功、下载失败和清理失败；将通用与专业双语译文检查限定为 transcript；扩展 FastAPI + PostgreSQL 集成场景，覆盖租约过期重领、拒绝阶段倒退、逐字稿替换和 Agent 交接 | 成员 4 确认兼容设计后逐项执行；本机 Python 全量为 `253 passed, 10 skipped`，Ruff、前端测试/类型检查/生产构建、`verify.sh`、敏感文件和 diff 检查通过。因本机无 `TEST_DATABASE_URL`，真实 PostgreSQL 组合测试明确跳过；PR 首轮数据库组合测试已通过，但两条纯单元测试因 CI 无 FFmpeg 失败，改用测试内音频抽取替身后，`backend-check`、`frontend-check` 和 `scaffold` 全部通过 | 初版远程链无 Adapter 仍进入 translate；重领 transcribe 会因本地音频准备回写 extract_audio，外层下载/清理失败也可能写错阶段；双语门禁把课件误当成逐字稿要求 translation | 采用兼容优先最小修复；翻译、课件解析和独立证据索引继续标为内部测试能力，未接入远程纵向链路；未下载模型，未新增后端接口或迁移，未接管成员 5 的通用 Agent 运行器 | `worker/pipeline.py`、`worker/runner.py`、`agent/orchestrator.py`、`agent/validators/evidence_gate.py`、`tests/unit/test_worker.py`、`tests/unit/test_agent.py`、`tests/integration/test_processing_api.py` |
| 2026-07-30 | 成员 4 + Codex | 按组长边界完成 M1 Worker 阶段一 | “后端由成员 3，通用 Agent 由成员 5；先做 Worker 内部能力与契约草案；模型先不下载” | 实现单 Worker 常驻轮询、有界退避、鉴权失败停机和停止后零写入；实现可替换翻译契约、语言检测和 translate 阶段；实现 PDF/PPTX 页级解析、确定性内存证据草稿、computer_ai/humanities Skill 与专业证据 gate；提交成员 3 后端契约提案 | 组长已人工确认职责边界和 M1 优先级，但尚未逐行审核本轮代码；自动化覆盖轮询、停止、翻译对齐、解析、证据定位和专业正反例。真实翻译模型、真实后端证据写入、第二段远程链路与失败重试尚未验收 | 初版设计混入后端与通用 Agent 职责，且重试可能在不回退阶段时偷偷重跑前段；修订为成员 4 阶段一内部实现，显式提交 fencing、版本化、交接和阶段回退需求。解析测试依赖在 dev 与 worker 分组重复声明，避免为现有 CI 安装 Whisper | 采纳边界内实现；不修改 backend、通用协调器或 Agent runner；不下载翻译模型，不把 fake adapter 算作双语通过；P0 只做文字/页码，PNG 与完整部署留 P1 | `worker/runtime.py`、`worker/stages/translate.py`、`worker/stages/parse_courseware.py`、`worker/stages/build_evidence_index.py`、`agent/skills/computer_ai.py`、`agent/skills/humanities.py`、`agent/validators/evidence_gate.py`、`worker/m1-backend-contract-proposal.md` |
| 2026-07-30 | 成员 3 + Codex | Day 4 复核、报告持久化与最小审计及 PR #22 审核修正 | “优先完成可演示 MVP：教师审核结论后，报告只能持久化已接受或已修改的结论。”；“按审核意见修复报告正文门禁、首次 PUT 并发和贡献归属” | 实现复核决定/历史、课堂报告 GET/PUT 及脱敏审计；审核修正后 PUT 只接收标题，正文由服务端按当前可报告结论生成，并以课堂行锁串行化首次创建 | 客户端正文绕过、modified 改写、驳回后正文/关联同步移除及两个独立 HTTP 会话并发首次 PUT 回归通过；合入最新 main 后端全量 184 项通过/1 项条件跳过，Ruff、前端测试、类型检查和生产构建通过 | 初版只过滤报告关联但仍信任客户端 Markdown，且查询后 INSERT 会并发触发唯一约束；单次 flush 直接播种外键还会造成顺序错误 | 服务端统一重建受控正文；复核与保存共用课堂行锁，报告始终与当前状态同步；PR #18/#19/#20 与 #22 贡献分别归属 | `backend/app/repositories/reviews.py`、`tests/integration/test_review_report_api.py` |
| 2026-07-30 | 成员 1 | 暂代成员 5 补齐 Agent 运行时集成 | “Worker 已能写回逐字稿，下一项优先补 Agent 自动接管；不要直接改 main，保留成员 5 原始归属” | Codex 协助设计并实现 Worker→Agent 原子交接、Agent 最小权限领取/续租、编排调用、结论写回、完成状态、安全失败和前后端分析契约对齐，并补充单元、接口与前端漂移测试 | 成员 1 决定采用确定性阶段交接而非用 ReAct 控制基础设施状态，确认服务令牌隔离、租约所有权、失败不写入半成品结论和中英分析按教师需求开启；Ruff、Python 全仓 `174 passed, 10 skipped`、前端测试/类型检查/构建、`verify.ps1`、敏感检查与 diff 检查通过 | 审查发现三项会破坏真实链路的问题：Agent 在 `analyze/running` 状态更新后被错误清除租约；FastAPI 空队列返回 `200 null` 而客户端只接受 204；前端无条件要求双语会让尚无翻译证据的任务必然失败 | 修复租约释放条件并增加续租测试；Worker/Agent 客户端同时接受 204 与 `200 null`；双语门禁改为由教师复盘要求明确触发。当前结果只算尚未合并分支的离线自动化，不冒充真实模型、专用 PostgreSQL 或完整 E2E；Agent 主责仍归成员 5 | `backend/app/api/tasks.py`、`backend/app/repositories/tasks.py`、`agent/job_store.py`、`agent/runner.py`、`frontend/src/components/baseline/ReviewTaskBaseline.tsx`、`tests/unit/test_agent.py`、`tests/integration/test_processing_api.py` |
| 2026-07-30 | 成员 1 | 协作补齐成员 4 的 B2 视频下载与 Worker 领取链路 | “成员 1—5 的工作均可协作，先处理当前最要紧的真实链路阻塞” | 定位并修复 Worker 未领取 `uploaded` 阶段任务、远程模式只支持本地 `object_root`、Whisper 日期版本上界写错三项阻塞；后端领取时签发限时只读地址，Worker 使用无服务令牌的独立客户端流式下载、核对大小与已验证 ETag 并清理临时文件 | Ruff 与全仓门禁通过（169 项通过、10 项条件跳过）；真实获授权英语视频完成 `B2 → 下载 → FFmpeg → Whisper tiny → PostgreSQL`，持久化 1 段 `[0, 28000]` ms 逐字稿，非法/重叠时间范围均为 0，任务停在 `transcribe / running / 1.0` 等待 Agent；当前任务临时目录残留为 0 | 原远程入口请求阶段不含 `uploaded`，即使启动也领不到新任务；领取契约只有对象地址，没有 B2 读取能力；`openai-whisper>=20250625,<2027` 因日期版本量级错误排除了全部可用版本；下载失败还会在进入 pipeline 前留下运行中任务并可能通过异常链暴露签名地址；最终审查进一步发现只核对大小无法阻止预签名 PUT 过期前的同大小对象替换 | 保留成员 4 对媒体流水线的主责归属，成员 1 只记录跨模块协作；长期 B2 密钥和教师 JWT 不进入 Worker，预签名 URL 不写日志；缺失或变化的 ETag 均拒绝并清理；真实转写已通过，后续 Agent 接管、翻译、课件解析和证据索引继续按未完成处理 | `backend/app/schemas/task.py`、`backend/app/api/tasks.py`、`worker/job_store.py`、`worker/runner.py`、`tests/unit/test_worker.py`、`tests/integration/test_processing_api.py` |
| 2026-07-30 | 成员 1 | 同步 Day 3 与 M1 项目说明 | “只更新 `.md` 和其他说明性文件；已合并代码算完成，其他对话实现标记为进行中” | 盘点 `main@1e52c9c` 的前端、后端、Worker、Agent 与测试事实；新增统一进度页并同步 README、v5 计划、验收矩阵、评分卡、模块指南、手工记录和报告材料 | 成员 1 确认状态口径；对照合并提交、API 路由、Worker/Agent 入口和现有测试记录，执行文档一致性扫描与 `git diff --check` | 多份指南仍把已经合并的上传、任务和结论接口写成未实现；同时必须防止把其他对话正在实现的 Worker/B2 集成提前写成完成 | 历史记录保留原结论并增加时间说明；当前文档统一区分已合并、局部验证、进行中和未通过，不修改业务代码 | `docs/current-progress.md`、`docs/acceptance-matrix.md`、`reports/group-report.md` |
...
| 2026-07-30 | 成员 1 + Codex | 统一本地开发环境与启动说明 | 汇总成员首次运行所需软件、环境变量、分服务启动、角色边界和常见故障；后续实现变化再同步更新 | AI 对照现有脚本、配置校验、Compose 和模块指南起草统一文档并接入索引 | 成员 1 发现成员 2 只启动前端后无法创建真实课堂，确认先新增项目内统一指南；真实密钥未提供或写入 | 现有说明分散，`start.ps1` 仍是占位入口；文档不能把分窗口启动描述成一键运行，也不能把示例秘密当真实配置 | 新增临时统一事实入口，明确 `localhost`、UUID、PostgreSQL/MinIO、B2 密钥边界和未完成限制；仅完成静态文档与骨架规则核验，尚未由新成员按全文复现 | `docs/local-development-setup.md`、`docs/documentation-index.md`、`README.md` |
| 2026-07-30 | 成员 1 + Codex | PR #27 Worker 合并前安全复核与修正 | 审核成员 4 媒体流水线是否满足证据来源、进程停机、课件隐私和资源限制门禁；保留原成果归属 | AI 审查差异、构造边界回归并协助最小修复；未代替成员 4 的流水线设计与主体实现 | 成员 1 要求审核后直接修正易改阻断项；自动化执行 Worker/翻译/证据/领域规则 112 项测试，并用独立 Python 运行验证 PPTX 讲者备注排除和异常压缩拒绝；合入最新 `main` 后，除课件测试文件外 Python 全仓 `243 passed, 11 skipped`，前端测试、类型检查、生产构建和 Ruff 通过 | 原实现把同一逐字稿片段同时登记为 `TRANSCRIPT` 与 `VIDEO`，可绕过视觉证据门禁；进程停止事件未传入活动租约；PPTX 讲者备注会进入证据且缺少解压规模门禁；贡献报告部分描述超出实现事实；最新 `main` 的团队隧道脚本还存在一处 Ruff 导入顺序错误 | 逐字稿只生成逐字稿证据；活动任务在安全检查点响应进程停止；PPTX 仅抽取幻灯片可见内容并拒绝超大解压体积/异常压缩比；同步纠正成员 4 贡献报告、保留协作日志双方记录并修正导入顺序；完整课件 pytest 因临时依赖下载未完成，不能虚报为已执行 | PR #27、`worker/runner.py`、`worker/stages/build_evidence_index.py`、`worker/stages/parse_courseware.py`、对应单元测试 |
| 2026-07-30 | 成员 1 + Codex | Cloudflare Quick Tunnel 临时团队联调入口 | 在最终托管方案未冻结前，为跨网络组员提供零费用短时联调；只暴露前端/BFF，不暴露后端、数据库和管理端口 | AI 实现可选访问码门禁、随机访问码、临时启动脚本、B2 精确 CORS 管理、静态安全契约测试与统一说明 | 成员 1 否决 Render 后确认先采用 Quick Tunnel；批准安装官方 `cloudflared` 并在本地预检通过后确认开启临时入口 | 真实启动先后发现三项未被静态测试覆盖的问题：`cloudflared` 会记录继承的访问码环境变量；受限端口检查会把已有 3000 前端误当成新门禁进程；反向代理后的 `Origin` 与内部 URL 不同。对应入口均在发给成员前关闭作废。B2 S3 接口还会丢弃 CORS 规则 ID，不能据 ID 清理 | `cloudflared` 改用干净子进程环境；前端改用独立随机端口和实例挑战值；来源校验改为拒绝 cross-site 并核对转发主机；B2 临时规则按唯一精确 Quick Tunnel 来源识别、保留其他规则且支持清理。最终入口经公网 307/401、正确登录、安全 Cookie、首页和后端健康代理验证通过 | `frontend/src/proxy.ts`、`frontend/src/app/team-access/`、`scripts/start-team-tunnel.ps1`、`scripts/configure-team-tunnel-cors.py`、`frontend/tests/team-access.test.mjs`、`docs/local-development-setup.md` |
...
## 2026-08-02：PR #28 真实证据读取契约修复

- **成员 2 原始成果**：提交视频下载地址、任务逐字稿、逐句编辑和课堂结论的
  前端 BFF/API Client/TypeScript 类型初稿，为证据工作台接入真实数据做准备。
- **成员 1 人工决策**：发现该分支落后于主分支且发生三处冲突后，决定由成员 1
  接管兼容性修复，并坚持保留主分支已经可用的素材删除、结论复核、报告和诚实
  Mock 边界。
- **AI 协助与人工复核**：Codex 对照 Python Schema 和 FastAPI 路由，修正空下载
  路由、错误动态路径、错误逐字稿字段和 `any` 返回类型；成员 1 负责最终审阅、
  Git 提交和 PR 合并决定。
- **验证结果**：前端 `npm test`、`npm run typecheck`、`npm run build` 均通过；
  该结果只证明真实数据接入边界已具备，不把仍显示 Mock 数据的证据工作台描述为
  已完成真实联调。

## 2026-08-03：成员 1 真实证据工作台集成

- **成员 1 人工决策**：在成员 2 已提供的真实证据读取契约上继续集成，选择按任务从
  后端恢复素材关系，不把 asset ID 或预签名视频地址持久化到浏览器；并要求真实链路与
  原有 Mock 演示严格分开。
- **AI 协助**：Codex 补充 owner-scoped 任务素材查询、前端 BFF/API Client，接通真实视频、
  带时间戳逐字稿、逐句人工校订、任务结论及接受/修改/驳回持久化，并补充相应测试。
- **人工边界**：本记录只说明实现与自动化核验；真实课堂的端到端人工体验、视觉意见和
  报告导出结果仍需成员 1 在本地服务启动后复核，不把自动化测试冒充教师试用。

## 2026-08-10：成员 1 真实报告工作流集成

- **成员 1 人工决策**：坚持报告正文只能由后端根据教师已接受或已修改的结论生成，前端
  仅开放标题修改；原有 `/reports/demo` 继续作为独立 Mock 演示，不混入真实课堂链路。
- **AI 协助**：Codex 对照后端 Schema、报告仓储和导出路由，补充报告导出 BFF、前端类型、
  真实报告载入/首次创建、标题保存、预览，以及 Markdown、HTML、PDF 三种服务端导出。
- **隐私与门禁**：预签名下载地址只保存在当前 React 内存状态，不写入浏览器持久化存储，
  页面不展示对象 key 或鉴权信息；未复核和已驳回结论仍由后端阻止进入正文与导出文件。
- **人工边界**：自动化测试与构建通过只能证明代码和契约一致，不能替代教师对真实报告内容、
  下载文件和整体任务体验的人工验收；该人工验收未执行前不得标记为已通过。

## 2026-08-10：真实报告会话恢复人工迭代

- **人工发现**：成员 1 合并真实报告工作流后直接打开报告深链接，看到笼统的“报告暂时无法
  载入”，并反馈该页面未能进入。日志确认页面请求为 200，但报告 BFF 返回 401。
- **人工决策**：不使用开发者控制台或手工 Cookie 绕过问题，而是在产品内提供可理解、可操作
  的会话恢复入口。
- **AI 协助**：Codex 补充 401 状态保留、同源演示会话恢复和自动重试，并增加回归测试；令牌
  仍只存在 HttpOnly Cookie，不进入前端 JavaScript 或浏览器持久化存储。
- **人工复验**：成员 1 点击“建立演示会话并重试”后成功进入真实报告页；日志确认
  `POST /api/auth/demo 200` 后紧接 `GET /api/classrooms/{id}/report 200`。当前报告无结论，
  因而本轮不能扩大为“含真实结论的报告门禁已人工通过”。

## 2026-08-10：单输入真实数据技术 E2E

- **成员 1 决策与监督**：选择只推进一条获授权真实视频，拒绝让多个历史 queued 任务
  同时消耗资源；确认继续使用既定本地模型 `qwen3.5:4b`，不引入新的云端数据流。
- **AI 协助**：Codex 盘点运行环境并启动 PostgreSQL、Worker 与 Agent；识别历史分析契约
  不兼容和旧 B2 对象 404 两类真实失败；重新完成预签名上传、后端 HEAD 核验和任务创建，
  监控 Worker→Agent 交接、模型分析、结论写回、教师复核、报告过滤及三格式导出。
- **人工判断**：成员 1 授权使用此前已用于本项目的本机原视频，并决定把本轮结果作为技术
  E2E 证据；所有 Git 分支、提交和最终合并仍由成员 1 的 Windows 账号执行。
- **隐私处理**：未在记录中保存课堂正文、完整逐字稿、预签名 URL、对象密钥、JWT、服务
  令牌或模型推理；只记录必要的任务/Trace 标识、状态、计数与 HTTP 结果。
- **验收边界**：真实链路已到达 accepted/modified 报告和 Markdown/HTML/PDF 下载，但操作
  由成员 1 监督下的公开业务 API 完成，不冒充非开发教师独立浏览器试用；第二输入、同任务
  修改后复测、自动翻译 Provider 和课件证据仍待完成。

## 2026-08-14：双语契约补救闭环

- **人工发现与决策**：成员 1 指出系统不仅要解释纯中文课堂选错双语的原因，也要让真正的
  非纯中文课堂补充上传内容；随后确认第一版采用带时间轴的中文译文 SRT/VTT。
- **AI 核查与实现**：Codex 发现原 Worker 完全不消费逐字稿资产，因而没有只做前端按钮；
  补充了私有对象下载、UTF-8 SRT/VTT 解析、中文与时间轴校验、ASR 时间覆盖对齐、
  `translate` 阶段恢复、前端上传核验和复用原视频创建新任务。
- **人工边界**：产品格式和是否合并仍由成员 1 决定；课堂视频、字幕正文、预签名 URL 与
  密钥均未写入文档或测试输出。当前结论来自自动化，不冒充真实 B2/浏览器人工验收。

## 2026-08-15：浏览器双语恢复实测与旧 Worker 隔离

- **人工发现与决策**：成员 1 在浏览器同时上传视频和译文后仍看到双语证据失败，要求系统
  既要解释错误，也要让教师真正完成补充上传；确认继续采用教师提供的定时 VTT，不把它
  表述成自动翻译。
- **AI 核查与协助**：Codex 只检查状态、计数、进程启动时间、对象大小和 ETag，定位到旧版
  Worker 抢先领取任务；清理经双重核对的旧进程后复用已上传资料重试。首次字幕下载出现
  暂时性失败，确认预签名对象可下载且大小/ETag 一致后再次重试。
- **人工复验**：成员 1 在浏览器打开替代任务并确认可用；Worker 重新生成 ASR，4 个片段
  均对齐教师译文，随后 Agent 成功。记录中不保存任务 ID、字幕正文、预签名 URL 或密钥。
- **后续改进**：为 Windows `start.ps1` 增加命名互斥锁及 PID/启动时间双校验进程清单；
  该保护只约束此后由统一脚本启动的实例，不冒充操作系统级服务管理或完整部署验收。
